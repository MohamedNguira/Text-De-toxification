...
def lemma(sentence: Union[str, List[str]]) -> List[str]:
    lem = WordNetlemmar()
    tokens = tokenize(sentence) if isinstance(sentence, str) else sentence
    # lemma
    tokens = [lem.lemma(t.strip().lower()) for t in tokens]
    return tokens
...
def delspaces(text: str) -> str:
    return re.sub('\s+', ' ', text)

def delextra(text: str) -> str:
...
def unibatch(strings: Iterable[str], nlp = None) -> List[str]: 
