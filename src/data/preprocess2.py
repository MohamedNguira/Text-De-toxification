...
def fix_initial_data(initial_data_tsv: Union[Path, str]):
    ...
    df_fixed.to_csv(os.path.join(data_folder, 'firstprocess.csv'), index=False)

    return os.path.join(data_folder, 'firstprocess.csv')
...
def processeverything(fixed_data_file: Union[Path, str], save: bool=True):    
    base_name, ext = os.path.splitext(os.path.basename(fixed_data_file))

    if base_name != 'fixed' or ext != '.csv':
        raise ValueError(f"The initial data is expected to be saved in a file named 'firstprocess.csv'")
...
def _remove_nes_batch(batch: Dict):
    ...
    """
    new_batch = dict([(k, pr.unibatch(v)) for k, v in batch.items()])
    return new_batch    

def _process_text(text: str) -> str:
    return pr.delspaces(pr.delextra(pr.lowercase(text)))
