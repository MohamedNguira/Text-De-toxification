...
from nltk.stem import WordNetlemmar

import src.data.preprocess1 as pr
...
def _prepare_sentence(sentence: str, stop_words_set: set) -> List[int]:
    """This function tokenizes, lemmas, removes stop words and return the tokens' ids by the Bert-distilled tokenizer. 
    """
    ...
    result =  [lem.lemma(t) for t in pr.tokenize(sentence) if t.isalpha() and t not in stop_words_set]
    return result
...
def get_toxicity_attributes(sentence: str, 
    ...
    s = _prepare_sentence(sentence, pr.stdstopwords())
