...
def _prepare_sentence(sentence: str, stop_words_set: set) -> List[int]:
    ...
    result =  [lem.lemma(t) for t in pr1.tokenize(sentence) if t.isalpha() and t not in stop_words_set]
    return result
...
def get_toxicity_attributes(sentence: str, 
    ...
    s = _prepare_sentence(sentence, pr1.stdstopwords())
